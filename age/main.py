...
Exports ``asgi_app`` plus the lifecycle hooks the hub loader looks for. Running this
file directly starts a standalone server on the same URL layout the hub publishes,
which is what the development workflow uses.
"""
...
def _standalone():
    ...
    root = FastAPI(title="Age (standalone)", lifespan=lifespan)

    @root.get("/", include_in_schema=False)
    async def _redirect() -> RedirectResponse:
        return RedirectResponse(url=f"{settings.base_path}/")

    root.mount(settings.base_path, asgi_app, name="age")
    return root, settings


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the Age world server on its own.")
    ...
    logger.info("Age on http://%s:%d%s/", args.host, args.port, settings.base_path)
