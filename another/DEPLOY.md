...
Первым делом открыть консоль браузера (F12 → Console). Админка грузит
криптографию с CDN `jsdelivr`, и если модуль не загрузился, то не сработает
вообще ничего — молча, без сообщения в интерфейсе. Типичные причины: нет
доступа к `cdn.jsdelivr.net` или в браузере блокировщик, режущий сторонние
скрипты.
