...
Restart контейнера на Hugging Face **не** подтягивает новые коммиты. Путь такой:
`git push` → Action `Sync to Hugging Face Space` зелёный → Space сам
пересобирает Docker (часто 15–30 минут) → Ctrl+F5 в админке. Если Action
красный — Space так и останется на старом `app.js`.
