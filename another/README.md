...
Практика вместо замысла: развернуть — [DEPLOY.md](DEPLOY.md), пустить свой
трафик через развёрнутое — [GETTING-STARTED.md](GETTING-STARTED.md). Второй
документ описывает только проверенное на работающем коде и отдельно
перечисляет, что пока не работает.

UI клиента — английский. Документация и комментарии в коде — русские.

## Статус

v1 по частям 1–5 **написана**. Control plane развёрнут на Hugging Face Space
внутри монорепозитория `cutaway`, воркер — на Cloudflare; Tier1-трафик идёт
через `/proxy` самого воркера. Клиент собирается из исходников, готовых
инсталляторов и сборки Flutter нет. Reality-сервер есть; в проде — когда
появится VPS. Очередь: [docs/release-plan.md](docs/release-plan.md).
