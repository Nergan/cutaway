...
**Состояние деплоя:** `another/.project-ignore` запрещает build, runtime и
deployment. Автоматические edge/installer workflows отключены; CI-тесты
сохранены. Для возобновления недостаточно снять паузу в этом документе —
необходимо также удалить kill switch и явно вернуть deploy-триггеры.
